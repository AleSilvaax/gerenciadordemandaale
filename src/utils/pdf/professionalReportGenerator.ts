...
const ACCENT_COLOR = '#FF8C00';     // Laranja para destaque
...
export const generateProfessionalServiceReport = async (
  ...
  try {
    ...
    const htmlString = `
      ...
      <head>
        ...
        <style>
          ...
          .page { padding: ${PAGE_MARGIN}; position: relative; min-height: 90vh; }
          .cover {
          ...
          .footer { position: fixed; bottom: 20px; width: 100%; text-align: center; font-size: 8pt; color: #aaa; }
        </style>
      </head>
      <body>
        <div class="cover">
      ...
      callback: (doc) => {
        ...
        for (let i = 1; i <= pageCount; i++) {
          ...
          if (i > 1) { // Não adiciona rodapé na capa
            doc.html(`<div class="footer">Página ${i} de ${pageCount}</div>`, {
        ...
        doc.save(fileName);
        logger.info(`Relatório V8 gerado: ${fileName}`, 'PDF');
      },
      autoPaging: 'slice',
      margin: [50, 50, 50, 50]
    });
    ...
    logger.error(`Erro ao gerar Relatório V8: ${error instanceof Error ? error.message : 'Desconhecido'}`, 'PDF');
