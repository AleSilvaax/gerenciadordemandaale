...
// import { processImage } from './imageProcessor'; // Linha removida para simplificar
...
// O código foi simplificado para usar diretamente o URL da imagem, pois o jspdf consegue processá-lo de forma assíncrona.
// Isso elimina a necessidade da função processImage separada, que causava o problema de "promessas não resolvidas".
// -------------------------------------------------------------------------------------------------------------------------------------
...
    
    // Usa um loop for...of para processar as fotos de forma sequencial e assíncrona
    for (const photo of service.photos) {
      let i = service.photos.indexOf(photo);
      if (i % photosPerRow === 0) {
...
      
      try {
        // Adiciona a imagem usando a URL, com tratamento de erro
        doc.addImage(photo.url, 'JPEG', xPos, currentY + 5, photoWidth, photoHeight);
      } catch (error) {
