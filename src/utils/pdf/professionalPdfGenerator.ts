...
import { processImage } from './imageProcessor'; // Importação da função corrigida
...
const createSignaturesSection = async (doc: jsPDF, service: Service, startY: number): Promise<number> => {
  ...
  if (service.signatures?.client) {
    ...
    try {
      // ✅ Chamada corrigida da função processImage para a assinatura do cliente
  ...
  if (service.signatures?.technician) {
    ...
    try {
      // ✅ Chamada corrigida da função processImage para a assinatura do técnico
...
const createPhotosSection = async (doc: jsPDF, service: Service, startY: number): Promise<number> => {
  ...
  if (service.photos && service.photos.length > 0) {
    ...
    for (let i = 0; i < service.photos.length; i++) {
      ...
      try {
        // ✅ Chamada corrigida da função processImage, passando a URL da foto
        const imageData = await processImage(service.photos[i].url);
        if (imageData) {
          doc.addImage(
            imageData,
            'JPEG',
