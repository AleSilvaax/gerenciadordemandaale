...
export const useServiceDetail = () => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // Conforme esperado pelo arquivo 'serviceMessaging.ts'
      const messageData: ServiceMessage = {
        senderId: user.id,
        senderName: user.name || "Usuário",
        senderRole: user.role || "tecnico",
        message: newMessage.trim(),
        // Propriedades adicionais que o tipo pode esperar, mas não são salvas diretamente
        id: '', 
      ...
      // Recarrega todos os dados para garantir que a nova mensagem apareça
