-- Primeiro, remover qualquer trigger/função existente que possa estar causando problemas
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
DROP TRIGGER IF EXISTS handle_new_user_trigger ON auth.users;

-- Remover funções antigas que podem estar em conflito
DROP FUNCTION IF EXISTS public.handle_new_user_robust();
DROP FUNCTION IF EXISTS public.handle_new_user_fixed();
DROP FUNCTION IF EXISTS public.handle_new_user_corrected();
DROP FUNCTION IF EXISTS public.handle_new_user_simple();

-- Criar função robusta para lidar com novos usuários
...
'Função robusta v2 para criar perfil de usuário com organização padrão sempre definida';